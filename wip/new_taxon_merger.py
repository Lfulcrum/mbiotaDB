...
def aggregate_at_taxon_level(df, taxon_level='genus', 
    ...
    if grouping_cols:
        ...
        grouping_cols = list(df.columns[:upper_index])
...
#    print(new_df.head())
    print(new_df.loc[(new_df['workflow_id'] == 32) & (new_df['sample_id'] == 3001), :])
    try:
...
if __name__ == '__main__':
    test_file = r'../data/test_data/taxon_merging/db_output.csv'
    ## TEST TO RAISE EXCEPTIONS
#    df = pd.read_csv(test_file)
#    aggregate_at_taxon_level(df, taxon_level='blah')
    
#    df = pd.read_csv(test_file)[[col for col in df.columns if col != 'kingdom']]
#    aggregate_at_taxon_level(df, taxon_level='kingdom')
#    aggregate_at_taxon_level(df, taxon_level='genus')
    
    ## TEST TO SUM COUNTS
#    df = pd.read_csv(test_file)[[col for col in df.columns if col not in ['seq_var_id', 'id', 'lineage_id']]]
    df = pd.read_csv(test_file)
